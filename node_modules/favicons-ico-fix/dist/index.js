...
function favicons(source, options = {}, next = undefined) {
  ...
  async function createFavicon(sourceset, properties, name, platformOptions) {
    if (path.extname(name) === ".ico") {
      const images = await Promise.all(properties.sizes.map(({
        width,
        height
      }) => createFavicon(sourceset, { ...properties,
        width,
        height,
        raw: true
      }, `${width}x${height}.rawdata`, platformOptions)));
    ...
    const contents = await µ.Images.composite(await canvas, await buffer, mergedProperties, offset);
    return {
      name,
      contents
    };
  ...
  async function create(sourceset) {
    ...
    if (options.icons.android && options.manifestMaskable && typeof options.manifestMaskable !== "boolean") {
      const maskableSourceset = await µ.General.source(options.manifestMaskable);
      responses.push(await createPlatform(maskableSourceset, "android_maskable"));
    }
