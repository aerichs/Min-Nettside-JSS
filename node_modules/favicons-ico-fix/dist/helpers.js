    ...
    Files: {
      create(properties, name) {
        ...
        if (name === "manifest.json") {
          ...
          properties.theme_color = options.theme_color; // Defaults to false, so omit the value https://developer.mozilla.org/en-US/docs/Web/Manifest/prefer_related_applications
          ...
          properties.icons.forEach(icon => {
            ...
            icon.purpose = options.manifestMaskable === true ? "any maskable" : "any";
          ...
          if (options.manifestMaskable && typeof options.manifestMaskable !== "boolean") {
            const maskableIcons = properties.icons.map(icon => ({ ...icon,
              src: icon.src.replace(/android-chrome-(.+)\.png$/, "android-chrome-maskable-$1.png"),
              purpose: "maskable"
            }));
    ...
    Images: {
      create(properties) {
        ...
        let image = sharp({
          create: {
            ...
            background: properties.transparent ? "#00000000" : properties.background
          }
        });
      ...
      async render(sourceset, properties, offset) {
        ...
        const nearestIcon = minByKey(sourceset, icon => {
          ...
          return [iconSideSize >= sideSize ? 0 : 1, Math.abs(iconSideSize - sideSize)];
        });
        ...
        return image.resize({
          ...
          fit: sharp.fit.contain,
          background: "#00000000",
          kernel: options.pixel_art && width >= metadata.width && height >= metadata.height ? "nearest" : "lanczos3"
        });
      },
      ...
      async maskImage(image, mask) {
        ...
        const maskBuffer = await sharp(mask).resize({
          width: meta.width,
          height: meta.height,
          fit: sharp.fit.contain,
          background: "#00000000"
        }).toColourspace("b-w").toBuffer();
      ...
      async overlay(image, coverPath) {
        const pipeline = sharp(image);
        const meta = await pipeline.metadata();
        const cover = await sharp(coverPath).resize({
          width: meta.width,
          height: meta.height,
          fit: sharp.fit.contain
        }).png().toBuffer();
        return await pipeline.composite([{
          input: cover,
          left: 0,
          top: 0
        }]).png().toBuffer();
      ...
      async composite(canvas, image, properties, offset) {
        ...
        if (properties.raw) {
          return await pipeline.toColorspace("srgb").raw({
            depth: "uchar"
          }).toBuffer({
